...
def test_successful_post_returns_true_and_sends_the_message_as_json(monkeypatch):
    ...
    assert captured["body"] == {"text": "hello world"}  # no "channel" key when unset


def test_channel_is_included_in_the_payload_when_configured(monkeypatch):
    monkeypatch.setenv("SLACK_WEBHOOK_URL", "https://hooks.slack.example/T000/B000/xxx")
    monkeypatch.setenv("SLACK_CHANNEL", "#sync-service-alerts")
    captured = {}
    ...
    assert slack.post("hello world") is True
    assert captured["body"] == {"text": "hello world", "channel": "#sync-service-alerts"}


def test_channel_omitted_from_payload_when_not_configured(monkeypatch):
    monkeypatch.setenv("SLACK_WEBHOOK_URL", "https://hooks.slack.example/T000/B000/xxx")
    monkeypatch.delenv("SLACK_CHANNEL", raising=False)
    captured = {}
